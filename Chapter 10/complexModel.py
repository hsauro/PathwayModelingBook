...
# To generate the grid, call: plotGrid (loadIntoPandas (toFit, pSamples))
def plotGrid (df):
    ...
    plt.savefig(r"grid.pdf")
    
np.random.seed (int (time.time()))

# Use this to reproduce the same run, commetn out otherwise
np.random.seed (132)
...
print ("Loading ground truth and experimental data......")
...
x_data = ground_truth[:, 0] # Extract the time column
...
# Set True to run the boostrap analysis
if True:
...
# Set True to run the python confidence limits tracer
if False:
    to = time.time()
...
# Set to True to run the MCMC algorithm, see Bayesian Inference chapter
if False:
    # Install corner to use this: pip install corner
